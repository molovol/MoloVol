...
bool Voxel::isProbeExcluded(const std::array<double,3>& vxl_pos, const double& r_probe, const double& radius_of_influence, const std::vector<Atom>& close_atoms){
  ...
  for (size_t i = 0; i < close_atoms.size(); i++){
    ...
    for (size_t j = i+1; j < close_atoms.size(); j++){
      ...
      for (size_t k = j+1; k < close_atoms.size(); k++){
        ...
        for (size_t l = k+1; l < close_atoms.size(); l++){
...
//  double dist_vxl_12 = unitvec_12 * vec_vxl; // voxel vector component along 12
  ...
  if (side_restr){
    vec_probe = vec_probe_plane + vec_probe_normal * ( std::signbit(vec_probe_normal*vec_vxl)? 1 : -1 );
  }
...
bool allAtomsClose(
    ...
    char n_atoms)
{
  /* TODO remove because unused unless there is a future use
